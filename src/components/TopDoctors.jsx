...
// ─── FIXES ───────────────────────────────────────────────────────────────────
// 1. Ek single IntersectionObserver for ALL cards (pehle har card ka alag observer tha)
// 2. Stagger animation CSS se — JS animation loop nahi
// 3. memo already tha — rakha
...
// ✅ Single observer for the whole grid
  ...
  return (
    <Link
      ...
      style={{
        ...
        transition: `opacity 0.45s ease ${index * 55}ms, transform 0.45s ease ${index * 55}ms, box-shadow 0.3s, translate 0.3s`,
      }}
      aria-label={`View ${doctor.name}'s profile`}
    >
      <div className="relative overflow-hidden bg-blue-50 dark:bg-gray-700 h-44 sm:h-48">
...
const TopDoctors = () => {
  ...
  // ✅ Single observer for entire grid — much cheaper than 10 observers
  useEffect(() => {
